...
A zone is a set of names and addresses for a domain like lab.example.com. 
...
The zone is lab.example.com.
The file is spaced out to look more like a table (that's another suggestion from https://tools.ietf.org/html/rfc1912[RFC 1912]). 
...
The zone is 1.168.192.in-addr.arpa.
...
Tell named to re-read its config with the command _systemctl reload named.service_.


== check your work 

Check the https://en.wikipedia.org/wiki/SOA_record[SOA (Start Of Authority) records]. 
